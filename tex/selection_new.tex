...
\todo{add necessary calibration, id, isolation notes citation}
...
\todo{find full details in the HGam common note or fiducial or couplings}

\subsubsection{Jet reconstruction and selections}
\todo{add necessary calibration, JVT, fJVT citations}
...
PFlow jet collection is still work-in-progress. From EMTopo fJVT studies, it has been seen that the scale-factor is significantly deviating from one and also the use of fJVT in the analysis brings a moderate improvement \todo{incorrect, now it's fixed and it's used and applied get the full description for Jet/Etmiss or fiducial note}.



\subsubsection{Electron selections}
\todo{remove, no need since our selection doesn't involve leptons}
...
\todo{Reformulate this part and integrate it with previous, no need to make old and new selection.}
...
\begin{description}
	...
	\item[Trigger]~
	\todo{add the single photon triggers, add the trigger eff and citations}
	\begin{itemize}
...
The per-selection \todo{fix text} for VBF process is inherited from old STXS 1.0 framework in 80 \ifb analysis, which is also based on the kinematic characters of the reconstructed particles \cite{ATLAS-CONF-2018-028}. A $qq\to Hqq$ final state is required, with the rapidity of Higgs should be within the barrel region $|\eta_{H}|<2.5$. The upper limit for jet energy is set to 200GeV \todo{are you sure this is applied, it's not in the table}. Di-jet system should have a large invariant mass $m_{jj}>400GeV$ and large pseudo-rapidity separation $|\Delta \eta_{jj}|>2.0$. A Zeppenfield $\eta$  cut is used to focus on VBF process $|\eta^{Zepp}|<5$. An additional BDT based categorization specifically for this analysis would be described in Section ~\ref{sec:Category}. 
